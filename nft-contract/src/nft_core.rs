...
impl NonFungibleTokenCore for Contract {
    ...
    fn nft_transfer_call(
        ...
        ext_non_fungible_token_receiver::nft_on_transfer(sender_id, previous_token.owner_id.clone(), token_id.clone(), msg, receiver_id.clone(), NO_DEPOSIT, env::prepaid_gas() - GAS_FOR_NFT_TRANSFER_CALL)
        .then(ext_self::nft_resolve_transfer(authorized_id, previous_token.owner_id, receiver_id, token_id, previous_token.approved_account_ids, memo, env::current_account_id(), NO_DEPOSIT, GAS_FOR_RESOLVE_TRANSFER)).into()
    }
    ...
    fn nft_token(&self, token_id: TokenId) -> Option<JsonToken> {
        if let Some(token) = self.tokens_by_id.get(&token_id){
            ...
            Some(JsonToken {token_id, owner_id: token.owner_id, metadata, approved_account_ids: token.approved_account_ids, royalty: token.royalty})
        }else{
            None
        }
    }
...
impl NonFungibleTokenResolver for Contract {
    ...
    fn nft_resolve_transfer(
        ...
        self.internal_remove_token_form_owner(&receiver_id, &token_id);
